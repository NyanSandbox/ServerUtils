...
class ExpCommand extends Command {
    ...
    public ExpCommand(String permission, String command, MessagesManager mm) {
    ...
    public void execute(CommandSender sender, boolean permission, Player player) {
        ...
        String format = super.mm.getColoredMessage("player.stats.exp");
        String exp    = String.valueOf(player.getExpToLevel());

        sender.sendMessage(StringUtils.replaceVariable(format, exp));
    }
}
