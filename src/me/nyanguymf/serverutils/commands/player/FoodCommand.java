...
class FoodCommand extends Command {
    ...
    public FoodCommand(String permission, String command, MessagesManager mm) {
    ...
    public void execute(CommandSender sender, boolean permission, Player player) {
        ...
        String format = super.mm.getColoredMessage("player.stats.food");
        String food   = String.valueOf(player.getFoodLevel());

        sender.sendMessage(StringUtils.replaceVariable(format, food));
    }
}
